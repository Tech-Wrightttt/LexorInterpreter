    ...
    void run() {
        while (cur < tokens.size() && tokens[cur].type != TokenType::END_OF_FILE) {
            ...
            if (t.type == TokenType::DECLARE) {
                ...
                cur++; if (tokens[cur].type == TokenType::COLON) cur++;
                while (cur < tokens.size() && tokens[cur].type != TokenType::NEWLINE && tokens[cur].type != TokenType::END_SCRIPT) {
                    Token p = tokens[cur++];
                    if (p.type == TokenType::IDENTIFIER) std::cout << vars[p.value];
                    else if (p.type == TokenType::INT_LIT || p.type == TokenType::FLOAT_LIT || p.type == TokenType::BOOL_LIT || p.type == TokenType::CHAR_LIT || p.type == TokenType::STRING_LIT) std::cout << p.value;
                    else if (p.type == TokenType::DOLLAR) std::cout << "\n";
                    else if (p.type == TokenType::LBRACKET) {
                        if (cur < tokens.size()) {
                            std::cout << tokens[cur].value; // Print escaped content
                            cur++;
                            if (cur < tokens.size() && tokens[cur].type == TokenType::RBRACKET) cur++; // Discard closing ]
                        }
                    }
                    if (cur < tokens.size() && tokens[cur].type == TokenType::AMPERSAND) cur++; // Consume & but do not print
                }
            }
            else cur++;
        }
